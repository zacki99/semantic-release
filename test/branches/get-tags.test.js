...
test('Get the valid tags', async t => {
  ...
  const result = await getTags({cwd, options: {branches: [{name: 'master'}], tagFormat: `v\${version}`}});
...
test('Get the valid tags from multiple branches', async t => {
  ...
  const result = await getTags({
    cwd,
    options: {branches: [{name: '1.x'}, {name: 'master'}, {name: 'next'}], tagFormat: `v\${version}`},
  });
...
test('Match the tag name from the begining of the string and the channel from the last "@"', async t => {
  ...
  const result = await getTags({cwd, options: {branches: [{name: 'master'}], tagFormat: `prefix@v\${version}`}});
...
test('Return branches with and empty tags array if no valid tag is found', async t => {
  ...
  const result = await getTags({
    cwd,
    options: {branches: [{name: 'master'}, {name: 'next'}], tagFormat: `prefix@v\${version}`},
  });
...
test('Get the highest valid tag corresponding to the "tagFormat"', async t => {
  ...
  t.deepEqual(await getTags({cwd, options: {branches: [{name: 'master'}], tagFormat: `\${version}`}}), [
  ...
  t.deepEqual(await getTags({cwd, options: {branches: [{name: 'master'}], tagFormat: `foo-\${version}-bar`}}), [
  ...
  t.deepEqual(await getTags({cwd, options: {branches: [{name: 'master'}], tagFormat: `foo-v\${version}-bar`}}), [
    {
      name: 'master',
  ...
  t.deepEqual(await getTags({cwd, options: {branches: [{name: 'master'}], tagFormat: `(.+)/\${version}/(a-z)`}}), [
    {
      name: 'master',
  ...
  t.deepEqual(await getTags({cwd, options: {branches: [{name: 'master'}], tagFormat: `2.0.0-\${version}-bar.1`}}), [
    {
      name: 'master',
  ...
  t.deepEqual(await getTags({cwd, options: {branches: [{name: 'master'}], tagFormat: `\${version}-bar.2`}}), [
...
test('Get the tag on branch where commits have been rebased', async t => {
  ...
  const result = await getTags({
    cwd,
    options: {branches: [{name: 'master'}, {name: 'next'}], tagFormat: `v\${version}`},
  });
