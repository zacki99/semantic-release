...
import {lts, prerelease, release} from '../../lib/definitions/branches';

test('A "lts" branch is identified by having a "range" property or a "name" formatted like "N.x", "N.x.x" or "N.N.x"', t => {
  t.true(lts.filter({name: '1.x.x'}));
  t.true(lts.filter({name: '1.0.x'}));
  t.true(lts.filter({name: '1.x'}));
  t.true(lts.filter({name: 'some-name', range: '1.x.x'}));
  t.true(lts.filter({name: 'some-name', range: '1.1.x'}));
  t.true(lts.filter({name: 'some-name', range: ''}));
  t.true(lts.filter({name: 'some-name', range: null}));
  t.true(lts.filter({name: 'some-name', range: false}));

  t.false(lts.filter({name: 'some-name'}));
  t.false(lts.filter({name: '1.0.0'}));
  t.false(lts.filter({name: 'x.x.x'}));
});

test('A "lts" branches must have a "range" property formatted like "N.x", "N.x.x" or "N.N.x"', t => {
  t.true(lts.branchValidator({name: 'some-name', range: '1.x.x'}));
  t.true(lts.branchValidator({name: 'some-name', range: '1.1.x'}));

  t.false(lts.branchValidator({name: 'some-name', range: '^1.0.0'}));
  t.false(lts.branchValidator({name: 'some-name', range: '>=1.0.0 <2.0.0'}));
  t.false(lts.branchValidator({name: 'some-name', range: '1.0.0'}));
  t.false(lts.branchValidator({name: 'some-name', range: 'wrong-range'}));
  t.false(lts.branchValidator({name: 'some-name', range: ''}));
  t.false(lts.branchValidator({name: 'some-name', range: null}));
  t.false(lts.branchValidator({name: 'some-name', range: false}));
});

test('The "lts" branches must have unique ranges', t => {
  t.true(lts.branchesValidator([{range: '1.x.x'}, {range: '1.0.x'}]));

  t.false(lts.branchesValidator([{range: '1.x.x'}, {range: '1.x.x'}]));
  t.false(lts.branchesValidator([{range: '1.x.x'}, {range: '1.x'}]));
});
